...
<template>
    <section
        id="contact"
        class="h-screen flex items-center contact-section relative overflow-hidden"
        style="background: transparent"
    >
        <!-- Decorative elements -->
        <div
            class="absolute top-12 right-12 w-20 h-20 bg-gradient-to-r from-green-500/10 to-emerald-500/10 rounded-full blur-xl"
        ></div>
        <div
            class="absolute bottom-12 left-12 w-24 h-24 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-full blur-xl"
        ></div>
...
.contact-icon {
    @apply w-16 h-16 bg-custom-gradient rounded-2xl flex items-center justify-center mx-auto mb-4 text-2xl;
}
...
.contact-button {
    @apply bg-custom-gradient text-white font-bold py-4 px-12 rounded-xl hover:shadow-xl hover:shadow-gradient-purple/30 transition-all duration-300 transform hover:scale-105;
}
</style>
